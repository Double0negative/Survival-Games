...
public class SpectatorEvents implements Listener {
	...
	public void onPlayerClickEvent(PlayerInteractEvent event) {
		...
		try {
			if (GameManager.getInstance().isSpectator(player)
					...
					|| GameManager.getInstance().isSpectator(player)
					&& player.isSneaking()
					&& (event.getAction() == Action.LEFT_CLICK_AIR || event
							.getAction() == Action.LEFT_CLICK_AIR)) {
				Player[] players = GameManager
						.getInstance()
						.getGame(
								GameManager.getInstance().getPlayerSpectateId(
										player)).getPlayers()[0];
				Game g = GameManager.getInstance().getGame(
						GameManager.getInstance().getPlayerSpectateId(player));
	 ...
	 * onEntityTarget(EntityTargetEvent event) { Player player = null; if
	 * (event.getTarget() instanceof Player) { player =
	 * (Player)event.getTarget(); } else return; if
	 * (GameManager.getInstance().isSpectator(player)) {
	 * event.setCancelled(true); } }
	 */
}
