...
public class LobbyManager {
	...
	/*
	 * class updater implements Runnable{ public void run(){ updateall(); } }
	 */
	...
	public void loadSign(int a) {
		try {
			...
			World w = Bukkit.getWorld(s.getString("sg-system.lobby.signs." + a
					+ ".world"));
			...
			s.set("sg-system.lobby.signs." + a, null);
			s.set("sg-system.lobby.signno",
					s.getInt("sg-system.lobby.signno") - 1);
		}
	}
	...
	public void loadSigns() {
		...
		// Bukkit.getScheduler().scheduleSyncRepeatingTask(GameManager.getInstance().getPlugin(),
		// new updater(), 100L, 20L);
	}
	...
	public void setLobbySignsFromSelection(Player pl, int a) {
		...
		if (sel.getNativeMaximumPoint().getBlockX()
				- sel.getNativeMinimumPoint().getBlockX() != 0
