...
public class ArenaDuplicator {
	...
	class background extends Thread {
		...
		public void run() {
			while (true) {
				System.out.println(fin + "/" + x + " " + (fin - prev) / 2 + " "
						+ (fin + 0.0) / (x + 0.0) * 100);
				prev = fin;
				try {
					sleep(2000);
	 ...
	 * NOT USED
	 * 
	 * Experimental Arena duplicator using multiple threads to copy the arena
	 * faster
	 * 
	 * Unfortunately it just crashes once its finished due to bukkit not being
	 * thread safe.
	 */
			...
			for (int x = min.getBlockX(); x < max.getBlockX(); x++) {
				for (int y = min.getBlockY(); y < max.getBlockY(); y++) {
					for (int z = min.getBlockZ(); z < max.getBlockZ(); z++) {
						...
						try {
							if (l1.getBlock().getTypeId() != l2.getBlock()
									.getTypeId()) {
	...
	public void startDupe(Vector v1, Vector v2) {
		...
		int factor = ManagementFactory.getOperatingSystemMXBean()
				.getAvailableProcessors();
		factor = 4;
		int xspan = v2.getBlockX() - v1.getBlockX();
		int maxx = GameManager.getInstance().getGame(1).getArena().getMax()
				.getBlockX();
		...
		background = new background(Math.abs(v2.getBlockX() - v1.getBlockX())
				* Math.abs(v2.getBlockY() - v1.getBlockY())
		...
		for (int a = 1; a <= factor; a++) {
			...
			Vector s1 = new Vector(sp1 < sp2 ? sp1 : sp2, y1 < y2 ? y1 : y2,
					z1 < z2 ? z1 : z2);
			Vector s2 = new Vector(sp1 > sp2 ? sp1 : sp2, y1 > y2 ? y1 : y2,
					z1 > z2 ? z1 : z2);
