...
public class UpdateChecker {
	...
	public void check(Player player, Plugin p) {
		...
		try {
			...
			data += "&" + URLEncoder.encode("ip", "UTF-8") + "="
			...
			URL url = new URL(
					"http://mc-sg.org/plugins/SurvivalGames/assets/updater/updatecheck.php");
			...
			if (b) {
				...
				player.sendMessage(ChatColor.DARK_RED
						+ "[SurvivalGames] Update Available!");
				player.sendMessage(ChatColor.DARK_AQUA + "Your version: "
						+ ChatColor.GOLD + v + ChatColor.DARK_AQUA
						+ " Latest: " + ChatColor.GOLD + in[1]);
				player.sendMessage(ChatColor.DARK_AQUA + in[2]);
				player.sendMessage(ChatColor.AQUA + "" + ChatColor.UNDERLINE
						+ in[3]);
				player.sendMessage(ChatColor.DARK_BLUE
						+ "--------------------------------------");
			...
			SurvivalGames.log(Level.WARNING,
					"[SurvivalGames] could not check for updates.");
		}
	}
}