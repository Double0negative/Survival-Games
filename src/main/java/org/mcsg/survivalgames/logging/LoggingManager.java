...
public class LoggingManager implements Listener {
	 ...
	 * blockChanged(BlockPhysicsEvent e){ logBlockCreated(e.getBlock()); }
	 */
	@EventHandler(priority = EventPriority.MONITOR)
	...
	/*
	 * @EventHandler(priority = EventPriority.MONITOR) public void
	 * blockChanged(BlockFromToEvent e){ logBlockDestoryed(e.getBlock());
	 * 
	...
	/*
	 * @EventHandler(priority = EventPriority.MONITOR) public void
	 * blockChanged(BlockRedstoneEvent e){ if(e.getBlock().getType() !=
	 * Material.REDSTONE_WIRE){ logBlockDestoryed(e.getBlock());
	 ...
	 * if(!SettingsManager.getInstance().getConfig().getBoolean("Random-Chest",
	 * true)){ if (e.getAction() != Action.RIGHT_CLICK_BLOCK) return;
	 * 
	 * Block clickedBlock = e.getClickedBlock(); int gameid =
	 * GameManager.getInstance().getPlayerGameId(e.getPlayer()); if (gameid ==
	 * -1) return; GameManager gm = GameManager.getInstance();
	 * 
	 * 
	 * if (GameManager.openedChest.get(gameid).contains(clickedBlock)) return;
	 * 
	 * if(GameManager.getInstance().getBlockGameId(clickedBlock.getLocation())
	 * != -1){
	 * 
	 * Chest c; if ((clickedBlock.getState() instanceof Chest)) { c =
	 * (Chest)clickedBlock.getState(); QueueManager.getInstance().add( new
	 * BlockData( GameManager.getInstance().getBlockGameId(c.getLocation()),
	 * c.getWorld().getName(), 0, (byte)0, c.getTypeId(), c.getData().getData(),
	 * c.getX(), c.getY(), c.getZ(), c.getBlockInventory().getContents()) ); } }
	 * } }
	 */
