...
public class GameManager {
	...
	public void addPlayer(Player p, int g) {
		Game game = getGame(g);
		if (game == null) {
			MessageManager.sendFMessage(Level.SEVERE, "error.input", p,
					"message-No game by this ID exist!");
			return;
		}
	...
	public void autoAddPlayer(Player pl) {
		...
		if (qg.size() == 0) {
			...
			MessageManager.sendMessage(Level.WARNING, "No games to join!", pl);
			return;
		}
	...
	public void LoadKits() {
		Set<String> kits1 = SettingsManager.getInstance().getKits()
