...
public class StatsManager {
	...
	// private HashMap<Player, PlayerStatsSession>pstats = new HashMap<Player,
	// PlayerStatsSession>();
	...
	public void addKill(Player p, Player killed, int arenaid) {
		...
		if (kslevel > 3) {
			MessageManager.broadcastFMessage(Level.INFO, "killstreak.level"
					+ (kslevel > 5 ? 5 : kslevel), "player-" + p.getName());
		} else if (kslevel > 0) {
			for (Player pl : GameManager.getInstance().getGame(arenaid)
					.getAllPlayers()) {
				MessageManager.sendFMessage(Level.INFO, "killstreak.level"
						+ (kslevel > 5 ? 5 : kslevel), pl,
						"player-" + p.getName());
			}
		}
	}
	...
	public void playerDied(Player p, int pos, int arenaid, long time) {
		 ...
		 * System.out.println("arenagetplayer null "+(arenas.get(arenaid).get(p)
		 * == null));
		 */
	...
	public void saveGame(int arenaid, Player winner, int players, long time) {
		...
		try {
			...
			PreparedStatement s2 = dbman.createStatement("SELECT * FROM "
			...
			if (time1 + 5000 < new Date().getTime()) {
				System.out
						.println("Your database took a long time to respond. Check the connection between the server and database");
			}
		...
		addSQL("INSERT INTO " + SettingsManager.getSqlPrefix()
				+ "gamestats VALUES(NULL," + arenaid + "," + players + ",'"
				+ winner.getName() + "'," + time + ")");
		...
		if (b) {
			try {
				...
				ResultSet tables = dbm.getTables(null, null,
						SettingsManager.getSqlPrefix() + "playerstats", null);
				ResultSet tables1 = dbm.getTables(null, null,
						SettingsManager.getSqlPrefix() + "gamestats", null);
