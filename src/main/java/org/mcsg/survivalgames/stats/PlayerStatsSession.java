...
public class PlayerStatsSession {
	...
	public void addkillStreak(int ks) {
		...
		ksbon = ksbon
				+ SettingsManager.getInstance().getConfig()
						.getInt("stats.points.killstreak.base")
				* (SettingsManager.getInstance().getConfig()
						.getInt("stats.points.killstreak.multiplier") + ks);
		int level = ks;
		if (level > 5) {
			level = 5;
		}
		...
		if (level < 4) {
			for (Player p : GameManager.getInstance().getGame(arenaid)
					.getAllPlayers()) {
				p.sendMessage(SettingsManager.getInstance().getConfig()
						.getString("stats.killstreak.level" + level)
	...
	public String createQuery() {
		calcPoints();
		String query = "INSERT INTO " + SettingsManager.getSqlPrefix()
				+ "playerstats VALUES(NULL,";
		query = query + gameno + "," + /*
										 * SettingsManager.getInstance().getConfig
										 * ().getString("sql.server-prefix")+
										 */arenaid + ",'" + player.getName()
				+ "'," + points + "," + position + "," + kills + "," + death
				+ ",";
		String killeds = "'";
		for (Player p : killed) {
		...
		query = query + "," + kslist.get(1) + "," + kslist.get(2) + ","
				+ kslist.get(3) + "," + kslist.get(4) + "," + kslist.get(5)
				+ ")";
	...
	public void died(int pos, long time) {
		...
		death = 1;
		position = pos;
		pppoints = GameManager.getInstance().getGame(arenaid)
				.getInactivePlayers();
	}
