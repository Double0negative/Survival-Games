...
public class HookManager {
	...
	private HashMap<String, Hook> hooks = new HashMap<String, Hook>();
	 ...
	 * for(String s: vars.keySet()){ str.replace("{$"+s+"}", vars.get(s)); }
	 * return str; }
	 */
	public boolean checkConditions(String str, String... args) {
		...
		for (String split : MessageUtil.replaceVars(str, args).split(";")) {
			...
			if (!flag) {
				System.out
						.println("[Survival Games][HookManager]Condition does not contian a compare operator: "
								+ split);
				return false;
			}
			try {
				...
				if (split.contains(">")) {
					if (Double.parseDouble(split.substring(0,
							split.indexOf(">")).trim()) > Double
							.parseDouble(split.substring(split.indexOf(">"))
									.trim())) {
					} else {
						return false;
					}
				...
				System.out
						.println("[Survival Games][HookManager]Error parsing value for: "
								+ split);
				return false;
			}
		}
	...
	/*
	 * public void runFHook(String hook, String... args){ HashMap<String,
	 * String>vars = new HashMap<String, String>(); for(String str: args){
	 * String[] s = str.split("|"); vars.put(s[0], s[1]); }
	 * 
	 * runHook(hook, vars); }
	 */
	...
	public void runHook(String hook, String... args) {
		...
		for (String str : c.getStringList("hooks." + hook)) {
			...
			if (checkConditions(split[2], args)) {
				if (p.equalsIgnoreCase("console") || split.length == 4
						&& Bukkit.getPlayer(p).hasPermission(split[3])
						|| split.length == 3) {
