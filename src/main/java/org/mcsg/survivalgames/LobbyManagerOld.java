...
public class LobbyManagerOld implements Listener {
	...
	class LobbySignUpdater implements Runnable {
		...
		public void run() {
			...
			/*
			 * try{ try{Thread.sleep(1000);}catch(Exception e){}
			 * }catch(Exception e){e.printStackTrace(); signs[0][0].setLine(1,
			 * ChatColor.RED+"ERROR");signs[0][0].setLine(1,
			 * ChatColor.RED+"Check Console");}
			 */
	...
	public void loadSigns() {
		...
		byte temp = ((Sign) new Location(p.getServer().getWorld(
				c.getString("sg-system.lobby.sign.world")), x1, y1, z1)
				.getBlock().getState()).getData().getData();
		// System.out.println("facing "+temp);
		if (temp == 3 || temp == 4) {
			l = new Location(Bukkit.getWorld(c
					.getString("sg-system.lobby.sign.world")), x1, y1, z1);
			inc = -1;
		} else {
			l = new Location(Bukkit.getWorld(c
					.getString("sg-system.lobby.sign.world")), x2, y1, z2);
			inc = 1;
		}
				...
				BlockState b = p
						...
						.getWorld(
								SettingsManager
										.getInstance()
										.getSystemConfig()
										.getString("sg-system.lobby.sign.world"))
						.getBlockAt(l).getState();
		...
		showMessage(new String[] { "", "Survival Games", "", "Double0negative",
				"iMalo", "mc-sg.org", "" });
	...
	public void setLobbySignsFromSelection(Player pl) {
		...
		if (sel.getNativeMaximumPoint().getBlockX()
				- sel.getNativeMaximumPoint().getBlockX() != 0
				&& sel.getNativeMaximumPoint().getBlockZ()
						- sel.getNativeMaximumPoint().getBlockZ() != 0) {
		...
		pl.sendMessage(ChatColor.GREEN
				+ "Lobby Status wall successfuly created");
		s.saveSystemConfig();
		loadSigns();
	...
	public void signShowMessage(String[] msg9, long wait) {
		...
		/*
		 * for(int y = signs.length-1; y!=-1; y--){ for(int a = 0; a<4; a++){
		 * 
		 * for(int x = 0; x!=signs[0].length; x++){
		 * 
		 * Sign sig = signs[y][x]; sig.setLine(a,
		 * "=================================================="); sig.update();
		 * 
		 ...
		 * for(int x = 0; x!=signs[0].length; x++){
		 * 
		 * Sign sig = signs[y][x]; if((y == signs.length - 1 && a ==0) || y == 0
		 * && a == 3){
		 * sig.setLine(a,"==========================================="); } else{
		 * sig.setLine(a, ""); signs[y][0].setLine(a,
		 * "|                         "); signs[y][0].update();
		 * signs[y][signs[0].length-1].setLine(a, "              |");
		 * signs[y][signs[0].length-1].update(); } sig.update();
		 * 
		 * } try{Thread.sleep(50);}catch(Exception e){}
		 * 
		 * } }
		...
		for (int c = 0; c < messagequeue.size(); c++) {
			...
			for (int a = msg.length - 1; a > -1; a--) {
				...
				for (char element : line) {
					...
					sig.setLine(lineno, sig.getLine(lineno) + element);
	...
	public void updateGameStatus() {
		...
		if (!SurvivalGames.UPDATED_CONFIG) {
			...
			try {
				...
				if (game.getMode() == Game.GameMode.STARTING) {
					signs[b][1].setLine(3, game.getCountdownTime() + "");
						...
						|| game.getGameMode() == Game.GameMode.FINISHING) {
						...
						signs[b][signno]
								.setLine(
										...
										(SurvivalGames.AUTH.contains(p
												.getName()) ? ChatColor.DARK_BLUE
												: ChatColor.BLACK)
												+ (p.getName()
														.equalsIgnoreCase(
																"Double0negative") ? "Double0"
														: p.getName()));
